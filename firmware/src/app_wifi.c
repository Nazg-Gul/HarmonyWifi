...
#define IS_WF_INTF(x) (strcmp(x, "MRF24W") == 0)
...
void APP_WIFI_Initialize(void)
{
  s_appData.state = APP_GREETING;

  s_app_set_param.conn.initConnAllowed = true;
  iwpriv_set(INITCONN_OPTION_SET, &s_app_set_param);
}

static bool greetings(void)
{
  SYS_CONSOLE_MESSAGE("SYS_Initialize: greetings\r\n");
...
static bool tcpip_init_wait(void)
{
  ...
  if (tcpipStat < 0) {
    ...
    SYS_CONSOLE_MESSAGE("APP: TCP/IP stack initialization succeeded!\r\n");
